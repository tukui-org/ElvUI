...
oUF:Factory(function(self)
	...
	local party
	if TukuiCF["raidframes"].partytarget ~= true then
		party = self:SpawnHeader("oUF_TukuiDPSParty", nil, "custom [@raid6,exists] hide;show", 
		...
		party = self:SpawnHeader("oUF_TukuiDPSParty", nil, "custom [@raid6,exists] hide;show", 
			'oUF-initialConfigFunction', ([[
				...
				self:SetHeight(%d)
				for i = 1, 5 do
					if ptarget == "oUF_TukuiDPSPartyUnitButton"..i.."Target" then
			...
			"showRaid", true, 
			"showSolo", false,
			"yOffset", TukuiDB.Scale(-27),
			'template', 'oUF_DPSPartyTarget'
		)	
	end
