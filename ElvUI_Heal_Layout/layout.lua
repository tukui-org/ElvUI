...
player:SetPoint("BOTTOMRIGHT", ElvuiActionBarBackground, "TOPLEFT", DB.Scale(-35),DB.Scale(205) + yoffset)
player:SetSize(player_width, player_height)

-- Target
local target = oUF:Spawn('target', "ElvHeal_target")
target:SetPoint("BOTTOMLEFT", ElvuiActionBarBackground, "TOPRIGHT", DB.Scale(35),DB.Scale(205) + yoffset)
target:SetSize(target_width, target_height)

-- Focus
local focus = oUF:Spawn('focus', "ElvHeal_focus")
if powerbar_offset ~= 0 then
	focus:SetPoint("TOPLEFT", ElvHeal_target, "BOTTOMLEFT", DB.Scale(9),DB.Scale(-42))
else
	focus:SetPoint("TOPLEFT", ElvHeal_target, "BOTTOMLEFT", 0,DB.Scale(-42))
end
focus:SetSize(smallframe_width, smallframe_height)

-- Target's Target
local tot = oUF:Spawn('targettarget', "ElvHeal_targettarget")
tot:SetPoint("TOPRIGHT", ElvHeal_target, "BOTTOMRIGHT", 0,DB.Scale(-42))
tot:SetSize(smallframe_width, smallframe_height)

-- Player's Pet
local pet = oUF:Spawn('pet', "ElvHeal_pet")
pet:SetPoint("TOPRIGHT", ElvHeal_player, "BOTTOMRIGHT", 0,DB.Scale(-42))
...
if C["unitframes"].showfocustarget == true then
	...
	focustarget:SetPoint("TOP", ElvHeal_focus, "BOTTOM", 0,DB.Scale(-32))
...
if ElvHeal_player.ThreatBar then
	if powerbar_offset ~= 0 then
		ElvHeal_player.ThreatBar:SetPoint("TOPLEFT", ElvHeal_target.Health, "BOTTOMLEFT", 0, -powerbar_offset + -DB.Scale(5))
	else
		ElvHeal_player.ThreatBar:SetPoint("TOPRIGHT", ElvHeal_target.Health, "BOTTOMRIGHT", 0, -(ElvHeal_target.Health:GetHeight() * 0.35) + -DB.Scale(8))
	end
end