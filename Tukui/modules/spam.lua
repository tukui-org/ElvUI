...
local function GM(self, event, arg1, arg2)
	local dist
	local whisp
	local rgb = "FFFFFF"
	if event == "CHAT_MSG_WHISPER" then dist = "WHISPER" whisp = arg2 end
	if event == "CHAT_MSG_BATTLEGROUND" then dist = "BATTLEGROUND" end
	if event == "CHAT_MSG_BATTLEGROUND_LEADER" then dist = "BATTLEGROUND_LEADER" end
	if event == "CHAT_MSG_BN_WHISPER" then dist = "BN_WHISPER" end
	if event == "CHAT_MSG_GUILD" then dist = "GUILD" end
	if event == "CHAT_MSG_OFFICER" then dist = "OFFICER" end
	if event == "CHAT_MSG_PARTY" then dist = "PARTY" end
	if event == "CHAT_MSG_PARTY_LEADER" then dist = "PARTY_LEADER" end
	if event == "CHAT_MSG_RAID" then dist = "RAID" end
	if event == "CHAT_MSG_RAID_LEADER" then dist = "RAID_LEADER" end
	if event == "CHAT_MSG_RAID_WARNING" then dist = "RAID_WARNING" end
	if event == "CHAT_MSG_SAY" then dist = "SAY" end
	if event == "CHAT_MSG_YELL" then dist = "YELL" end
	
	if dist ~= nil then
		rgb = RGBPercToHex(ChatTypeInfo[dist].r,ChatTypeInfo[dist].g,ChatTypeInfo[dist].b)
	end
	
	if arg2 == "Elv" then
		self:AddMessage("|TInterface\\ChatFrame\\UI-ChatIcon-Blizz.blp:0:2.2:0:0|t|cff"..rgb.."[|cffF58CBAElv|r|cff"..rgb.."]: "..arg1.."|r")
		return true
	end
end

ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_BATTLEGROUND", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_BATTLEGROUND_LEADER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_BN_WHISPER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_GUILD", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_OFFICER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_PARTY", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_PARTY_LEADER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_RAID", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_RAID_LEADER", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_RAID_WARNING", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_SAY", GM)
ChatFrame_AddMessageEventFilter("CHAT_MSG_YELL", GM)