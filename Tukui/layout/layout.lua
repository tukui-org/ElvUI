...
local mini = CreateFrame("Frame", "TukuiMinimap", Minimap)
TukuiDB.CreatePanel(mini, TukuiDB.Scale(144 + 4), TukuiDB.Scale(144 + 4), "CENTER", Minimap, "CENTER", -0, 0)
mini:ClearAllPoints()
mini:SetPoint("TOPLEFT", TukuiDB.Scale(-2), TukuiDB.Scale(2))
mini:SetPoint("BOTTOMRIGHT", TukuiDB.Scale(2), TukuiDB.Scale(-2))
...
if TukuiCF["actionbar"].enable == true then
	...
	TukuiDB.AnimGroup(TukuiActionBarBackgroundRight, TukuiDB.Scale(350), 0, 0.4)
...
TukuiDB.AnimGroup(ChatLBackground, TukuiDB.Scale(-375), 0, 0.4)
TukuiDB.AnimGroup(ChatRBackground, TukuiDB.Scale(375), 0, 0.4)
...
if TukuiCF["datatext"].battleground == true then
	infoleft:SetScript("OnMouseDown", function(self) 
		if shownbg == true then 
			TukuiDB.SlideOut(TukuiInfoBattleGroundL) 
			TukuiDB.SlideOut(TukuiInfoBattleGroundR) 
			shownbg = false 
		else 
			TukuiDB.SlideIn(TukuiInfoBattleGroundL) 
			TukuiDB.SlideIn(TukuiInfoBattleGroundR) 
			shownbg = true 
		end 
	end)
	...
	TukuiDB.AnimGroup(TukuiInfoBattleGroundL, 0, TukuiDB.Scale(-150), 0.4)
	TukuiDB.AnimGroup(TukuiInfoBattleGroundR, 0, TukuiDB.Scale(-150), 0.4)
...
ChatLBackground.anim:HookScript("OnFinished", function()
	...
	TukuiDB.StopFlash(TukuiInfoLeft.shadow)
end)
...
if TukuiCF["chat"].rightchat == true then
	...
	ChatRBackground.anim:HookScript("OnFinished", function()
		...
		TukuiDB.StopFlash(TukuiInfoRight.shadow)
	end)
end
