...
local function GetOptionsTable_ClassBar(updateFunc, groupName, numUnits)
	...
	elseif groupName == 'player' then
		...
		config.args.additionalGroup = ACH:Group(L["Additional Power"], nil, 20, nil, function(info) return E.db.unitframe.classAdditionalBar[info[#info]] end, function(info, value) E.db.unitframe.classAdditionalBar[info[#info]] = value updateFunc(UF, groupName, numUnits) end)
