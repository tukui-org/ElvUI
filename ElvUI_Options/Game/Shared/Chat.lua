...
General.args.socialQueueMessages = ACH:Toggle(L["Quick Join Messages"], L["Show clickable Quick Join messages inside of the chat."], 11, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
...
General.args.mentorshipIcon = ACH:Toggle(L["Mentorship Icon"], L["Display Mentorship status icon before names."], 14, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
General.args.recentAllyIcon = ACH:Toggle(L["Recent Ally Icon"], L["Display Recent Ally icon after names in chat (excludes whispers and emotes)."], 15, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
General.args.timerunningIcon = ACH:Toggle(L["Timerunning Icon"], L["Display Timerunning seasonal icon before names."], 16, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
...
General.args.scrollDownInterval = ACH:Range(L["Scroll Interval"], L["Number of time in seconds to scroll down to the bottom of the chat window if you are not scrolled down completely."], 22, { min = 0, max = 120, step = 1 })
General.args.numScrollMessages = ACH:Range(L["Scroll Messages"], L["Number of messages you scroll for each step."], 23, { min = 1, max = 12, step = 1 })
General.args.maxLines = ACH:Range(L["Max Lines"], nil, 24, { min = 10, max = 5000, step = 1 }, nil, nil, function(info, value) E.db.chat[info[#info]] = value CH:SetupChat() end)
General.args.editboxHistorySize = ACH:Range(L["Editbox History"], nil, 25, { min = 5, max = 50, step = 1 })
General.args.resetHistory = ACH:Execute(L["Reset Editbox History"], nil, 26, function() CH:ResetEditboxHistory() end)
General.args.editBoxPosition = ACH:Select(L["Chat EditBox Position"], L["Position of the Chat EditBox, if datatexts are disabled this will be forced to be above chat."], 27, { BELOW_CHAT = L["Below Chat"], ABOVE_CHAT = L["Above Chat"], BELOW_CHAT_INSIDE = L["Below Chat (Inside)"], ABOVE_CHAT_INSIDE = L["Above Chat (Inside)"] }, nil, nil, nil, function(info, value) E.db.chat[info[#info]] = value CH:UpdateEditboxAnchors() end)
