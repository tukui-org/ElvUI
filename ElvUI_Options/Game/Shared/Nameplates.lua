...
local function GetUnitAuras(unit, auraType)
	...
	group.args.stackAuras = ACH:Toggle(L["Stack Auras"], L["This will join auras together which are normally separated. Example: Bolstering and Force of Nature."], 2, nil, nil, nil, nil, nil, E.AuraContainer)
	group.args.desaturate = ACH:Toggle(L["Desaturate Icon"], L["Set auras that are not from you to desaturated."], 3, nil, nil, nil, nil, nil, E.AuraContainer)
	...
	group.args.generalGroup.args.sortMethod = ACH:Select(L["Sort By"], E.AuraContainer and L["Method to sort by.\n\n|cffff3333Note:|r Duration and Player are approximated by Time Remaining and Index."] or L["Method to sort by."], 16, { TIME_REMAINING = L["Time Remaining"], DURATION = L["Duration"], NAME = L["Name"], INDEX = L["Index"], PLAYER = L["Player"] })
	...
	group.args.sourceGroup = ACH:Group(L["Source Text"], nil, 20, nil, function(info) return E.db.nameplates.units[unit][auraType].sourceText[info[#info]] end, function(info, value) E.db.nameplates.units[unit][auraType].sourceText[info[#info]] = value NP:ConfigureAll() end, E.AuraContainer)
...
local function GetUnitSettings(unit, name)
	...
	group.args.aurasGroup.args.smartAuraPosition = ACH:Select(L["Smart Aura Position"], L["Will show Buffs in the Debuff position when there are no Debuffs active, or vice versa."], 1, C.Values.SmartAuraPositions, nil, nil, nil, nil, E.AuraContainer)
