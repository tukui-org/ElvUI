...
General.showMount = ACH:Toggle(L["Current Mount"], L["Display current mount the unit is riding."], 6, nil, nil, nil, nil, nil, nil, not E.Mists)
...
General.moneyLines = ACH:Toggle(L["Money Lines"], L["Replaces Blizzards price lines with custom ones.\n\n|cffff3333Note:|r Required to prevent issues with MoneyFrame."], 11, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
General.moneyHide = ACH:Toggle(L["Money Hide"], L["Hides Blizzards price lines."], 12, nil, nil, nil, nil, nil, function() return not E.db.tooltip.moneyLines end, not (E.Retail or E.Forever))
General.fadeOut = ACH:Toggle(L["Fade Out"], L["Fade out the tooltip when it disappears, instant otherwise. Cursor anchored tooltips are unaffected."], 13, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
General.allowTagPVP = ACH:Toggle(L["Allow Tag: PVP"], nil, 14)
General.allowTagFaction = ACH:Toggle(L["Allow Tag: Faction"], nil, 15)
General.spacer1 = ACH:Spacer(20)
General.colorAlpha = ACH:Range(L["OPACITY"], nil, 21, { isPercent = true, min = 0, max = 1, step = 0.01 }, nil, nil, function(info, value) E.db.tooltip[info[#info]] = value; S:StyleTooltips() end)
General.xOffset = ACH:Range(L["X-Offset"], nil, 22, { min = -200, max = 200, step = 1 })
General.yOffset = ACH:Range(L["Y-Offset"], nil, 23, { min = -200, max = 200, step = 1 })
General.anchorToBags = ACH:Select(L["Anchor to Bags"], L["Tooltip gets anchored to bags when they are shown."], 24, { TOPLEFT = L["TOPLEFT"], TOPRIGHT = L["TOPRIGHT"], DISABLED = L["Disabled"] }, nil, nil, nil, nil, function() return E.db.tooltip.cursorAnchor end, function() return not E.private.bags.enable end)
...
General.modifierGroup.args.includeWarband = ACH:Toggle(L["Include Warband"], nil, 7, nil, nil, nil, nil, nil, nil, not (E.Retail or E.Forever))
...
General.mythicPlus = ACH:Group(L["Mythic+ Data"], nil, -1, nil, nil, nil, nil, not (E.Retail or E.Forever))
...
General.healthBar.args.short = ACH:Toggle(L["Short Values"], nil, 2, nil, nil, nil, nil, function(_, value) E.db.tooltip.healthBar.short = value end, BarDisabled, not (E.Retail or E.Forever))
