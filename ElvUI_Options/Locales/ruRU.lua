...
L["^A blacklist filter is only effective against filters that come after it in the priority list. It will not block anything from the filters before it."] = "^Фильтр типа черный список работают только на на фильтры, которые идут после них в списке приоритета. Они не будут блокировать ничего из фильтров перед ними.|r"
...
L["*CastByUnit:|r ^Auras cast by the unit of the unitframe or nameplate (so on target frame it only shows auras cast by the target unit).|r"] = "*Принадлежит юниту:|r ^Ауры, наложенные юнитом рамки или индикатора (наприер, на фрейме цели будут показаны только ауры, наложенные целью).|r"
