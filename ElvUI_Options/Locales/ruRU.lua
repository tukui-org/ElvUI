...
L["Adjust the scale of the minimap and also the pins. Eg: Quests, Resource nodes, Group members"] = "Изменяет масштаб миникарты и значков на ней: квестоы, ресурсов, членов группы."
L["Alive"] = "Живой"
...
L["Dead or Ghost"] = "Мерт или призрак"
...
L["Enables the five-second-rule ticks for Mana classes and Energy ticks for Rogues and Druids."] = "Включает метки \"правила 5 секунд\" для клоассов с маной и метки энергии для разбойников и друидов."
L["Energy/Mana Regen Tick"] = "Метка регена маны/энергии"
L["Equipped"] = "Надето"
...
L["Has NPC Title"] = "Есть тиутл НИП"
L["Has Stealable"] = "Можно украсть"
L["Headers"] = "Заголовки"
L["Hide Icon"] = "Скрыть иконку"
L["Highlight Color Style"] = "Стиль подсветки"
L["Horde"] = "Орда"
L["If enabled then the filter will activate if the unit is casting anything."] = "Если включено, фильтр применится если юнит что-то кастует."
L["If enabled then the filter will activate if the unit is channeling anything."] = "Если включено, фильтр применится если юнит поддерживает заклинание."
L["If enabled then the filter will activate if the unit is not casting anything."] = "Если включено, фильтр применится если юнит ничего не кастует."
L["If enabled then the filter will activate if the unit is not channeling anything."] = "Если включено, фильтр применится если юнит не поддерживает заклинание."
L["If enabled then the filter will only activate when the unit has a stealable buff(s)."] = "Если включено, фильтр применится только когда на юните есть баффы, которые можно украсть."
L["If enabled then the filter will only activate when the unit has no stealable buff(s)."] = "Если включено, фильтр применится только когда на юните нет баффов, которые можно украсть."
L["If enabled then the filter will only activate when the unit is in your Party."] = "Если включено, фильтр применится только когда юнит в вашей группе."
L["If enabled then the filter will only activate when the unit is in your Raid."] = "Если включено, фильтр применится только когда юнит в вашем рейде."
L["If enabled then the filter will only activate when the unit is not in your Party."] = "Если включено, фильтр применится только когда юнит не в вашей группе."
L["If enabled then the filter will only activate when the unit is not in your Raid."] = "Если включено, фильтр применится только когда юнит не в вашем рейде."
L["If enabled then the filter will only activate when you have a target."] = "Если включено, фильтр применится только когда у вас есть цель."
L["Import"] = "Импорт"
L["In Combat Label"] = "Ярлык в бою"
L["In Pet Battle"] = "В битве питомцев"
L["Indicator Expiring"] = "Идникатор: окончание"
...
L["Known Spells"] = "Выученные заклинани"
...
-- L["Max Level"] = true --Moved to core locales
-- L["Minimap Cluster"] = true --Moved to core locales
...
-- L["Quest Log"] = true --Moved to core locales
...
L["Slots"] = "Слоты"
L["Soulbinds"] = "Медиумы"
-- L["Spell Haste"] = true --Moved to core locales
...
--L["Time Alert Frame"] = true --Moved to core locales
L["Time Text"] = "Текст времени"
L["Timer Only"] = "Только таймер"
L["Title"] = "Звание"
--L["Torghast Choice Toggle"] = true --Moved to core locales
...
-- L["If you have an icon or aurabar that you don't want to display simply hold down shift and right click the icon for it to disapear."] = "Если у Вас есть иконка или полоса аур, которую Вы не хотите отображать, просто shift+ПКМ на ней для занесения в черный список (она пропадет)." --Should be in core
...
L["Target units on mouse down rather than mouse up.\n|cffff3333Note:|r If Clique is enabled, this option only effects ElvUI frames if they are not blacklisted in Clique."] = "Выделять цели при нажати, а не отпускании кнопки мыши.\n|cffff3333Внимание:|r Если включен Clique, то опция будет работать только если рамки ElvUI не заигнорены в нем."
L["Target"] = "Цель"
...
L["This setting mimics the older style of Replace Blizzard Fonts, with a more static unified font sizing."] = "Эта опция косит под старый стиль замены шрифтов с более статичным и унифицированным размером шрифтов"
...
L["Up"] = "Вверх"
L["Upgrade Icon"] = "Иконка апгрейда"
