...
local function Enable(self)
	...
	if element then
		...
		if element.TopIndicator and element.TopIndicator:IsObjectType('Texture') and not element.TopIndicator:GetTexture() then
			...
			-- This comment and the next can be removed, just here for personal notes until I talk to simpy
		...
		if element.RightIndicator and element.RightIndicator:IsObjectType('Texture') and not element.RightIndicator:GetTexture() then
			...
			-- This comment and the next 2 can be removed, just here for personal notes until I talk to simpy
			-- element.RightIndicator:SetTexCoord(0, 1, 1, 1, 0, 0, 1, 0)	--Rotates Texture 90 degress counter clockwise (Up arrow to face left)
			-- texture:SetTexCoord(URx, URy, LRx, LRy, ULx, ULy, LLx, LLy)	-- Flip Texture Horizontally
