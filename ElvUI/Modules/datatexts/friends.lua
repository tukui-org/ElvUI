...
local C_Map_GetBestMapForUnitPlayer = function() return C_Map_GetBestMapForUnit("player") end
...
local function OnEnter(self)
	...
	local zonec, classc, levelc, realmc, info, grouped, shouldSkip, mapID
		...
		for i = 1, #friendTable do
			...
			if info[5] then
				...
				if not shouldSkip then
					...
					if mapID and (E:GetZoneText(mapID) == info[4]) then zonec = activezone else zonec = inactivezone end
					classc, levelc = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[info[3]], GetQuestDifficultyColor(info[2])
						...
						if not shouldSkip then
							if info[6] == wowString then
								...
								if IsShiftKeyDown() then
									mapID = C_Map_GetBestMapForUnitPlayer()
									if mapID and (E:GetZoneText(mapID) == info[15]) then zonec = activezone else zonec = inactivezone end
