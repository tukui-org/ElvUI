...
function mod:UpdateElement_NPCTitle(frame)
	if self.db.showNPCTitles and not self.db.units[frame.UnitType].healthbar.enable and not UnitIsPlayer(frame.unit) and not UnitPlayerControlled(frame.unit) and (not self.db.alwaysShowTargetHealth or not UnitIsUnit("target", frame.unit)) then
