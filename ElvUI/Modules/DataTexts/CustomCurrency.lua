...
local function RegisterNewDT(currencyID)
	...
	if name then
		...
		--Register datatext
		DT:RegisterDatatext(name, nil, {"CHAT_MSG_CURRENCY", "CURRENCY_DISPLAY_UPDATE"}, OnEvent, nil, nil, OnEnter, nil, name)
...
function DT:RegisterCustomCurrencyDT(currencyID)
	if currencyID then
		...
		for _, info in pairs(E.global.datatexts.customCurrencies) do
			...
			DT:RegisterDatatext(info.NAME, nil, {"CHAT_MSG_CURRENCY", "CURRENCY_DISPLAY_UPDATE"}, OnEvent, nil, nil, OnEnter, nil, info.NAME)
			--Get the currency index for this currency, so we can use it for a tooltip
