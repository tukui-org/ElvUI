...
function UF:Configure_HealthBar(frame)
	...
	local POWERBAR_HEIGHT = (frame.POWERBAR_HEIGHT or 0) * 0.5
	...
	local POWERBAR_BORDER = UF.SPACING + POWERBAR_HEIGHT -- this is meant to have UF.SPACING
	local BOTTOM_SPACING = BORDER_SPACING + frame.BOTTOM_OFFSET
	local CLASSBAR_YNOSPACING = BORDER_NOSPACING - CLASSBAR_YOFFSET
	...
	local PVPINFO_NOSPACING = BORDER_NOSPACING - PVPINFO_WIDTH
	...
	if frame.ORIENTATION == 'LEFT' then
		...
		elseif frame.USE_MINI_POWERBAR and frame.POWERBAR_SHOWN then
			...
			health.WIDTH = WIDTH_PVPINFO - PORTRAIT_SPACING
			health.HEIGHT = HEIGHT_YCLASSBAR - POWERBAR_BORDER
		else
			health:Point('BOTTOMLEFT', frame, 'BOTTOMLEFT', PORTRAIT_SPACING, BOTTOM_SPACING)
