...
function UF:Configure_Auras(frame, which)
	...
	auras.size = settings.sizeOverride ~= 0 and settings.sizeOverride
		or (
			(
				(
					frame.UNIT_WIDTH
					- (settings.spacing * (auras.num / settings.numrows - 1))
					- ((UF.thinBorders or E.twoPixelsPlease) and 0 or 2)
				) / auras.num
			) * settings.numrows
		)
	...
	auras.initialAnchor = UF.SideAnchor[settings.anchorPoint] and E.InversePoints[settings.anchorPoint]
		or (UF.GrowthPoints[settings.growthY] .. UF.GrowthPoints[settings.growthX])
	...
	auras.xOffset = x
		+ settings.xOffset
		+ (settings.attachTo == "FRAME" and frame.ORIENTATION ~= "LEFT" and frame.POWERBAR_OFFSET or 0)
...
function UF:SortAuras()
	if self.auraSort and #self.active > 1 then
		sort(self.active, UF.SortAuraFunc)
	end
...
function UF:PostUpdateAura(_, button)
	...
	if button.isDebuff then
		...
		elseif
			db.auraByDispels
			and button.debuffType
			and E.BadDispels[button.spellID]
			and E:IsDispellableByMe(button.debuffType)
		then
	...
	if
		button.needsUpdateCooldownPosition
		and (button.Cooldown and button.Cooldown.timer and button.Cooldown.timer.text)
	then
			...
			-- Special Filters
				...
				local found = (
					...
					and (
						...
						or (
							name == "BlizzardNameplate"
							and (nameplateShowAll or (nameplateShowSelf and (isPlayer or myPet)))
						)
					)
				)
					-- Blacklists
					or (
							(name == "blockCastByPlayers" and castByPlayer)
							...
							or (name == "blockNotDispellable" and not canDispel)
						)
						and 0
...
function UF:AuraUnchanged(
	...
	if
		...
		or a.Icon ~= icon
		or a.Count ~= count
		or a.debuffType ~= debuffType
		or a.duration ~= duration
		or a.expiration ~= expiration
		or a.source ~= source
		or a.isStealable ~= isStealable
		or a.nameplateShowPersonal ~= nameplateShowPersonal
		or a.spellID ~= spellID
		or a.canApplyAura ~= canApplyAura
		or a.isBossDebuff ~= isBossDebuff
		or a.castByPlayer ~= castByPlayer
		or a.nameplateShowAll ~= nameplateShowAll
	then
		a.name, a.Icon, a.Count, a.debuffType, a.duration, a.expiration, a.source, a.isStealable, a.nameplateShowPersonal, a.spellID, a.canApplyAura, a.isBossDebuff, a.castByPlayer, a.nameplateShowAll =
			name,
			icon,
			count,
			debuffType,
