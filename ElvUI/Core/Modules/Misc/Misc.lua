...
local GetFactionInfo = C_Reputation.GetFactionDataByIndex or GetFactionDataByIndex
local UnitIsGroupLeader = UnitIsGroupLeader
local ExpandAllFactionHeaders = C_Reputation.ExpandAllFactionHeaders or ExpandAllFactionHeaders
local SetWatchedFactionIndex = C_Reputation.SetWatchedFactionByIndex or SetWatchedFactionByIndex
	...
	if messagetype == 'FACTION' then
		...
		if data and faction ~= data.name then
			...
			for i = 1, GetNumFactions() do
				...
				if faction == factionData.name then
