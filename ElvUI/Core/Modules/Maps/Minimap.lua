...
if E.Retail or E.Cata then
	...
	tinsert(menuList, {
		text = _G.LFG_TITLE,
		microOffset = E.Retail and "LFDMicroButton" or "LFGMicroButton",
		func = function()
			if E.Retail then
...
tinsert(menuList, {
	text = _G.HELP_BUTTON,
	microOffset = not E.Retail and "HelpMicroButton" or nil,
	bottom = true,
	func = function()
		_G.ToggleHelpFrame()
	end,
...
function M:UpdateIcons()
	...
	if not next(IconParents) then
		...
		if indicator then
			M:SaveIconParent(indicator)
		end
		if craftingFrame then
			M:SaveIconParent(craftingFrame)
		end
		if mailFrame then
		...
		if difficultyGuild then
			M:SaveIconParent(difficultyGuild)
		end
		if difficulty then
	...
	if not noCluster then
		...
		if difficultyGuild then
			M:HandleDifficulty(difficultyGuild, true)
		end
...
function M:UpdateSettings()
	...
	local locationFont, locaitonSize, locationOutline =
		LSM:Fetch("font", E.db.general.minimap.locationFont),
