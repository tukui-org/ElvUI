...
local function updateAura(element, unit, index, offset, filter, isDebuff, visible)
	...
	if LCD and not UnitIsUnit("player", unit) then
		...
		name, icon, count, debuffType, duration, expiration, source, isStealable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, modRate, effect1, effect2, effect3 =
		...
		if fromRange or auras.createdButtons > auras.anchoredButtons then
			...
			* to   - the offset of the last aura button to be (re-)anchored (number)
			...
			(auras.SetPosition or SetPosition)(
				auras,
				fromRange or auras.anchoredButtons + 1,
				toRange or auras.createdButtons
			)
	...
	if buffs then
		...
		if fromRange or buffs.createdButtons > buffs.anchoredButtons then
			(buffs.SetPosition or SetPosition)(
				buffs,
				fromRange or buffs.anchoredButtons + 1,
				toRange or buffs.createdButtons
			)
