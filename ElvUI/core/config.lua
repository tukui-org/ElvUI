...
function E:Grid_Create()
	if not grid then
		grid = CreateFrame('Frame', 'ElvUIGrid', self.UIParent)
	...
	local size = 1
	local width = E.UIParent:GetWidth()
	local ratio = width / E.UIParent:GetHeight()
	local height = E.UIParent:GetHeight() * ratio
