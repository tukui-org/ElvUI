...
local function Shared(self, unit)
	...
	if unit == "player" then
		...
		if ElvCF["castbar"].unitcastbar == true then
			...
			if ElvCF["castbar"].castermode == true then
				...
				if db.swingbar ~= true then
					...
					castbar:SetPoint("BOTTOMRIGHT", ElvuiActionBarBackground, "TOPRIGHT", ElvDB.Scale(-2), ElvDB.Scale(14))
				end
			else
		...
		if db.swingbar == true then
			...
			Swing:SetStatusBarColor(unpack(ElvCF["media"].bordercolor))
			Swing:GetStatusBarTexture():SetHorizTile(false)
			self.Swing = Swing
			if ElvCF["castbar"].castermode == true then
				self.Swing:SetWidth(ElvuiActionBarBackground:GetWidth()-ElvDB.Scale(4))
				self.Swing:SetPoint("BOTTOM", ElvuiActionBarBackground, "TOP", 0, ElvDB.Scale(4))
			else
				self.Swing:SetWidth(original_width)
				if self.Castbar then
					self.Swing:SetPoint("TOPRIGHT", self.Castbar, "BOTTOMRIGHT", 0, ElvDB.Scale(-6))	
				else
					if powerbar_offset ~= 0 then
						self.Swing:SetPoint("TOPRIGHT", self.Health, "BOTTOMRIGHT", 0, -powerbar_offset + -ElvDB.Scale(5))
					else
						self.Swing:SetPoint("TOPRIGHT", self.Health, "BOTTOMRIGHT", 0, -(original_height * 0.35) + -ElvDB.Scale(8))
					end							
				end
			end
			
			self.Swing:SetHeight(ElvDB.Scale(4))
			
			self.Swing.bg = CreateFrame("Frame", nil, self.Swing)
			self.Swing.bg:SetPoint("TOPLEFT", ElvDB.Scale(-2), ElvDB.Scale(2))
			self.Swing.bg:SetPoint("BOTTOMRIGHT", ElvDB.Scale(2), ElvDB.Scale(-2))
			self.Swing.bg:SetFrameStrata("BACKGROUND")
			self.Swing.bg:SetFrameLevel(self.Swing:GetFrameLevel() - 1)
			ElvDB.SetTemplate(self.Swing.bg)
			self.Swing.bg:SetBackdropBorderColor(unpack(ElvCF["media"].altbordercolor))
		end
...
if ElvCF["unitframes"].charportrait == true then
	player:SetPoint("BOTTOM", ElvuiActionBarBackground, "TOPLEFT", ElvDB.Scale(-20),ElvDB.Scale(40+yOffset))
else
	player:SetPoint("BOTTOMLEFT", ElvuiSplitActionBarLeftBackground, "TOPLEFT", 0,ElvDB.Scale(40+yOffset))
end
...
if ElvCF["unitframes"].charportrait == true then
	target:SetPoint("BOTTOM", ElvuiActionBarBackground, "TOPRIGHT", ElvDB.Scale(20),ElvDB.Scale(40+yOffset))
else
	target:SetPoint("BOTTOMRIGHT", ElvuiSplitActionBarRightBackground, "TOPRIGHT", 0,ElvDB.Scale(40+yOffset))
end
target:SetSize(target_width, target_height)

-- Focus
local focus = oUF:Spawn('focus', "ElvDPS_focus")
focus:SetPoint("BOTTOMLEFT", ElvDPS_target, "TOPRIGHT", ElvDB.Scale(-35),ElvDB.Scale(120))
focus:SetSize(smallframe_width, smallframe_height)

-- Target's Target
local tot = oUF:Spawn('targettarget', "ElvDPS_targettarget")
tot:SetPoint("BOTTOM", ElvuiActionBarBackground, "TOP", 0,ElvDB.Scale(40+yOffset))
tot:SetSize(smallframe_width, smallframe_height)

-- Player's Pet
local pet = oUF:Spawn('pet', "ElvDPS_pet")
pet:SetPoint("BOTTOM", ElvDPS_targettarget, "TOP", 0,ElvDB.Scale(15))
pet:SetSize(smallframe_width, smallframe_height)
pet:SetParent(player)

-- Player's Pet's Target
if ElvCF["unitframes"].pettarget == true then
	local pettarget = oUF:Spawn('pettarget', "ElvDPS_pettarget")
	pettarget:SetPoint("BOTTOM", ElvDPS_pet, "TOP", 0,ElvDB.Scale(8))
...
if db.showfocustarget == true then
	...
	focustarget:SetPoint("BOTTOM", ElvDPS_focus, "TOP", 0,ElvDB.Scale(15))
...
if ElvDPS_player.ThreatBar then
	if powerbar_offset ~= 0 then
		ElvDPS_player.ThreatBar:SetPoint("TOPLEFT", ElvDPS_target.Health, "BOTTOMLEFT", 0, -powerbar_offset + -ElvDB.Scale(5))
	else
		ElvDPS_player.ThreatBar:SetPoint("TOPRIGHT", ElvDPS_target.Health, "BOTTOMRIGHT", 0, -(ElvDPS_target.Health:GetHeight() * 0.35) + -ElvDB.Scale(8))
	end
end